...
final class DHLWC_Email {
    ...
    public function send_stage(WC_Order $order, $stage, array $data = array()) {
        if ($this->settings['tracking_email_enabled'] !== 'yes') { return; }
        $subject = $this->replace($this->settings['email_subject_' . $stage] ?? '{site_name} - Kargo güncellemesi', $order, $stage, $data);
        $body = $this->replace($this->settings['email_body_' . $stage] ?? '', $order, $stage, $data);
        $to = $order->get_billing_email();
        if (!$to) { return; }
        $mailer = WC()->mailer();
        $content = $this->html($order, $stage, $subject, $body, $data);
        $mailer->send($to, $subject, $content, array('Content-Type: text/html; charset=UTF-8'));
    }
    ...
    private function html(WC_Order $order, $stage, $heading, $plain_body, array $data) {
        ...
        if (!empty($data['tracking_url'])) {
            echo '<p><a href="' . esc_url($data['tracking_url']) . '">' . esc_html__('Kargo takibini aç', 'dhl-ecommerce-for-woocommerce') . '</a></p>';
        }
        echo '</div>';
    ...
    private function replace($text, WC_Order $order, $stage, array $data) {
        ...
        $vars = array(
            ...
            '{tracking_line}' => $tracking_url ? 'Takip bağlantısı: ' . $tracking_url : '',
